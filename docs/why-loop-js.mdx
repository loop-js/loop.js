...
Claude Code; loop.js is the same convergence machinery as an API — `Loop.define` /
`loop.run` / typed events, embedded in your product instead of your terminal.** Everything
a loop needs to reach *done* — an independent verdict, durable state, declared guards, real
scheduling — in one npm package, driving the same Claude agents underneath.
