...
| `ExecuteCtx` | the deliberately starved per-round context a function Prompt receives | [Loop.define](/api/define#prompt) |
| `ExecuteSpec` / `VerifySpec` | per-phase binding: `prompt?`, `model?`, `permissions?` | [Loop.define](/api/define#loopconfig) |
